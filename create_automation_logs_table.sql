-- ===================================================================
-- PHASE 2: AUTOMATION LOG MANAGEMENT - DATABASE SETUP
-- ===================================================================
-- This script creates the automation_logs table and related indices
...
CREATE TABLE automation_logs (
    ...
    -- User related fields (using TEXT to match actual Supabase auth.users.id type)
    student_id TEXT,
...
CREATE POLICY "Users can view their own automation logs" ON automation_logs
    FOR SELECT USING (
        auth.uid()::text IN (target_user_id::text, student_id::text, teacher_id::text)
    );
...
-- Create automation rules table for storing automation rules in database
...
-- Create trigger for automation_rules
...
-- Note: These are commented out due to potential type mismatches between UUID and TEXT
-- Uncomment and modify as needed based on your actual table schema

-- For courses table (if it exists and uses UUID)
-- ALTER TABLE automation_logs 
-- ADD CONSTRAINT fk_automation_logs_course_id 
-- FOREIGN KEY (course_id) REFERENCES courses(id);

-- For user_profiles table (if user IDs are consistent type)
-- ALTER TABLE automation_logs 
-- ADD CONSTRAINT fk_automation_logs_student_id 
-- FOREIGN KEY (student_id) REFERENCES user_profiles(id);

-- ALTER TABLE automation_logs 
-- ADD CONSTRAINT fk_automation_logs_teacher_id 
-- FOREIGN KEY (teacher_id) REFERENCES user_profiles(id);

-- ALTER TABLE automation_logs 
-- ADD CONSTRAINT fk_automation_logs_target_user_id 
